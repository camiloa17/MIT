...
exports.consultaExamenCompletoCupos = async ()=>{
  ...
  'dia' as tabla,
  (dia.cupo_maximo-(select count(*) from reserva as res_dia where res_dia.examen_en_dia_RW_uuid=diarw.uuid and ((res_dia.fecha_venta IS NOT NULL) OR(res_dia.en_proceso=1 and (res_dia.fecha_reserva + INTERVAL 10 MINUTE > NOW()) )) )) as disponible_dia
  ,semana.*
...
'semana' as tabla1, 
(sls.cupo_maximo-(select count(*) from reserva as res where res.examen_en_semana_LS_uuid=exsls.uuid and((res.fecha_venta IS NOT NULL) OR(res.en_proceso=1 and (res.fecha_reserva + INTERVAL 10 MINUTE > NOW()) )))) as disponible_sem
...
where
  ...
  and (sls.cupo_maximo-(select count(*) from reserva as res where res.examen_en_semana_LS_uuid=exsls.uuid and((res.fecha_venta IS NOT NULL) OR(res.en_proceso=1 and (res.fecha_reserva + INTERVAL 10 MINUTE > NOW()) ))))>0
...
and (dia.cupo_maximo-(select count(*) from reserva as res_dia where res_dia.examen_en_dia_RW_uuid=diarw.uuid and ((res_dia.fecha_venta IS NOT NULL) OR(res_dia.en_proceso=1 and (res_dia.fecha_reserva + INTERVAL 10 MINUTE > NOW()) )) ))>0 
